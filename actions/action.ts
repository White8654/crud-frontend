...
// actions.ts
const apiUrl = "https://crud-backend-qxg3.onrender.com"; 
...
export const listTables = async (): Promise<string[]> => {
  return await apiRequest('/tables', 'GET');
};
...
// Authentication
export const authenticateUser = async (alias: string): Promise<any> => {
  return await apiRequest('/authenticate', 'POST', { alias });
};
...
export const addItem = async (
  ...
  await apiRequest('/item', 'POST', { tableName, ...itemData });
};
...
export const updateHead = async(
  ...
  await apiRequest('/rename-field', 'PUT', { tableName, oldFieldName,newFieldName});
};
...
export const getItemById = async (
  ...
  return await apiRequest(`/item/${tableName}/${id}`, 'GET');
};
...
export const deleteItem = async (
  ...
  await apiRequest(`/item/${tableName}/${id}`, 'DELETE');
};
